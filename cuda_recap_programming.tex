...
\noindent\fbox{
   ...
   {\fontfamily{pct}\selectfontarr} - the address of the 0'th element 
   ...
   \textit{reset} our index $j$ to 0 and increment $i$ (if $j=N-1$). However, in a flatened 2D case, in order to 
...
\newline
\newline
\newline Okay, now we can attack the code. From line 13 to 21, we are initializing the data and 
...
\subsubsection{Asynchronous memory}
  ...
  the memory allocation time. 
    The asynchronous memory allocation may reduce the time of a program execution, as it is asynchronous. And 
    non-blocking. This concept of asynchronous memory allocation involves the concept of streams, discussed 
    further. 
  Therefore the main part of this functionality be discussed in the CUDA streams section [REEEEEFF].
  ...
  many kernels, with many memory copies. Let's have a look at an abstract example, witout going into details, of what is the kernels 
  doing. 
  \begin{listing}
  \begin{minted}[frame=single, framesep=1mm, linenos=true]{cuda}
